...
pub struct Database {
    // Clone is safe here because rusqlite::Connection is NOT clone,
    // so we store a pathless shared connection by using Rc internally later.
    // For v0: keep single-owner and pass Database by move where needed.
    // (We keep Clone off in actual type; see note below.)
    conn: Connection,
}
